...
- Server-to-client door pack synchronization
...
The mod creates a `tarindoor/` directory in the game folder after the first launch. In single-player, place custom door ZIP files there and restart the game. On a dedicated server, install the ZIP files only in the server's `tarindoor/` directory; clients receive the definitions, textures, sounds, animations, names, and recipes automatically while connecting.
...
> The mod pre-registers 64 stable custom-door slots. Slot assignments are stored in `tarindoor/slots.json`; do not edit this file while a world is running. Restart the server after adding or changing ZIP files. Clients need the mod and its dependencies, but do not need copies of the ZIP files.
>
> When upgrading from a version that registered blocks as `custom_train_door:<pack_id>_door`, the mod migrates matching blocks, block entities, item stacks, structures, and assembled Create contraptions to their stable `tarindoor_XX_door` slots while loading. Keep the same door ZIPs installed on the server for the first upgraded load, and back up the world before upgrading. Data whose matching door pack is missing is deliberately left unmapped instead of being replaced by the wrong door.
>
> **Synchronization limits:** 32 MiB per ZIP and 128 MiB total.
...
The mod and its dependencies are required on both the client and the server. Custom door ZIP files are required only on the server.
...
首次启动后，游戏目录会生成 `tarindoor/` 文件夹。单人游戏仍将自定义门 ZIP 放入该文件夹并重启游戏。专用服务器只需把 ZIP 放入服务端的 `tarindoor/` 文件夹；玩家连接时会自动收到门定义、纹理、声音、动画、名称和配方，无需手动分发门包。
...
> 模组会预注册 64 个固定的自定义门槽位，分配关系保存在 `tarindoor/slots.json`。世界运行期间不要编辑该文件；添加或修改 ZIP 后请重启服务器。客户端仍需安装本模组及依赖，但不需要安装门包 ZIP。
>
> 从旧版升级时，模组会在读取过程中把匹配的方块、方块实体、物品堆、结构及已组装的 Create 载具，由 `custom_train_door:<门包ID>_door` 自动迁移到固定的 `tarindoor_XX_door` 槽位。首次升级加载时请确保服务端仍安装相同门包 ZIP，并务必先备份世界。缺少对应门包的数据会保持未映射状态，不会被静默替换成错误车门。
>
> **同步限制：**单个 ZIP 最大 32 MiB，全部门包合计最大 128 MiB。
...
客户端与服务端均需安装本模组及依赖；自定义门包 ZIP 只需安装在服务端。
